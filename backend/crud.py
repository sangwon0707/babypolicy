...
def create_user(supabase: Client, user: schemas.UserCreate, hashed_password: str):
    ...
    profile_data = {"user_id": user_id, "name": user.name or user.email.split("@")[0]}
...
def search_relevant_chunks(
    supabase: Client, query_embedding: list[float], top_k: int = 5
):
    """
...
def create_conversation(
    supabase: Client, user_id: str, title: str = "New Conversation"
):
    conversation_data = {"user_id": user_id, "title": title}
...
def get_posts(
    supabase: Client, skip: int = 0, limit: int = 10, category_id: str = None
):
    query = (
...
def create_post(supabase: Client, post: schemas.PostCreate, author_id: str):
    post_data = {**post.dict(), "author_id": author_id}
...
def create_comment(
    supabase: Client, comment: schemas.CommentCreate, post_id: str, author_id: str
):
    comment_data = {**comment.dict(), "post_id": post_id, "author_id": author_id}
